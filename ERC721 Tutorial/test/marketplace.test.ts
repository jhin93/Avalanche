...
contract('Marketplace', ([contractDeployer, creator, buyer, secondBuyer]) => {
    ...
    /*
    Afterwards, we simply test every statement which we make inside the function just like we did for the collectible.test.ts.
    We do the same for the cancelListing() function. For the buyItem() function things are not that different. 
    The interesting part is the final it() where we check the balances of the seller, buyer and creator after the second sale, since for the first one the seller is equal to the creator:
    */
    ...
    describe('Buy a NFT.', async () => {
        ...
        // For the buyItem() function things are not that different. 
        // The interesting part is the final it() where we check the balances of the seller, buyer and creator after the second sale, since for the first one the seller is equal to the creator:
